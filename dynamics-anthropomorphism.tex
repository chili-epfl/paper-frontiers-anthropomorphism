...
projection of already known cognitive models onto the robot. We discuss this
...
\emph{potentially} facing an agent endowed with ``human-level intelligence'' to
take McCarthy's words.

This is a transient state that vanishes as soon as the human observe that the
behaviour of the robot is \emph{predictable} and possibly \emph{deceptive} (for
instance, it talks but does not understand when we talk to him; it has eyes but
does not recognize everyday objects I show to him; etc.).

\paragraph{Disruptive behaviors}

The model also represents \emph{disruptive behaviors} as local
increases of anthropomorphic effects in the familiarization phase (and at a
lesser extend, in the stabilization phase) (Figure~\ref{fig:dynamics}).
...
understood by the user, we count the behavior as \emph{disruptive}.

Section~\ref{sec:disruptive} propose a typology of disruptive behaviors and
discusses the possible consequences on the level of anthropomorphism, which may
both increase if the user perceives (rightfully or not) a form of
intentionality, or decrease if the unexpected behavior is perceived as a
failure.
