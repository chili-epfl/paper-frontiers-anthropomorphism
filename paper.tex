...
Consequently, we propose a holistic understanding of anthropomorphism as a phenomenon which is determined by multiple factors:

\begin{enumerate}

\item ~the \textbf{characteristics of the robot's design} (degree of anthropomorphic form),
\item ~the \textbf{psychological determinants in the human user} (see
\cite{epley_seeing_2007}),
\item ~the \textbf{characteristics of the context} (situation) in which the interaction occurs, and
\item ~the fact of the user getting \textbf{familiar with the robot over time} (getting used to a system the more and longer one interacts with it)
