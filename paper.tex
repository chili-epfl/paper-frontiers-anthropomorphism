...
the artificial agent, and enable effective (human-like and 
...
robot~\citep{fink_anthropomorphism_2012}. But \emph{anthropomorphism} in general
...
complex bonds that human users build with robots.
...
\begin{figure}[h!]
        ...
        \end{subfigure}
    \caption{Usage and perception of the Roomba vacuum-cleaning robot over a 6 months period. Qualitative data suggest that people's activities related to the robot as well as their perception of the robot change over time. Overall, a rather low social engagement of the users with the robot, and few perception of human-likeness in the robot were found.}
    \label{fig:roomba}
\end{figure}
...
\begin{figure}[h!]
        ...
        \begin{subfigure}[b]{0.48\columnwidth}
            ...
            \caption{Qualitative level of anthropomorphic projections ($y$-axis) versus the actual interactions with the robot ($x$-axis) are negatively correlated.}
            \label{fig:qualitative-score}
        \end{subfigure}
        \caption{Children`s interactions with the Ranger robot toy box, and their anthropomorphic projections onto the robot. Overall, large variations of both children`s behavior and their perception of the robot were found but seemed to be correlated to each other.}
    \label{fig:ranger}
\end{figure}
...
\citep{fink_anthropomorphism_2012} In the following, we might use the terms \texitit{form} and \textit{design} interchangeably.
...
We now first present an artifact-centered perspective on anthropomorphism, and outline how 
robotics uses \textit{anthropomorphic design} to encourage 
people to interact in a social way with robots.
...
\citep{norman_design_1988,guthrie_bottles_2000}. Interestingly, from its very beginning the image of a robot exists as an outstanding challenge to mankind \citep{replicant_????}. Starting from when the term \texit{robot} first appeared in a play by Karel \^{C}apek in the 1920ies, robots were man-made beings, created as the \textit{``shadow of man''} (and sometimes resembling a human being in appearance).
...
intentions and other mental states to others \citep{premack1978_does, leslie_pretense_1987, Frith2003}.
...
mind  get activated
\citep{krach_can_2008}. \footnote{\hl{Ha! This says that anthropomorphism can also occur on a behavioral level, yes! :)}} 
Implication of human brain area involved in the inference of other's mental
states has also been shown in response to viewing
non-anthropomorphic/non-humanoid robotic gadgets whose behaviour
...
At lower, more automatic, level, robots have been shown to elicit resonnance behaviors in the
human brain. Resonnance behaviors \citep{Rizzolatti1999} are mechanisms by which the brain areas involved in the internal representations of an action, an emotion or a sensation are equally recruited during the perception of another individual performing the same action or experiencing the same emotion or sensation.  
The neurons showing resonnant properties have been called mirror neurons and
...
the participants were explicitely instructed to pay attention to the robot's
emotional expression, motor mirror neurons gets activated.
...
sufficient. Low-level mechanisms of the brain that allow humans to map other's
motor behaviors into their own repertoire are equally elicited when perceiving a
robot's action. It is not yet clear whether human are processing emotional
signal from a robot the same way as they do for human beings. 
