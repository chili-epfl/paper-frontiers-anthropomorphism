...
robots is \textit{anthropomorphism} -- people's tendency to perceive human-like 
characteristics in non-human agents. While this is a commonly discussed trait of 
human-robot interaction (HRI), the phenomenon paradoxically appears to lack formal 
...
humans anthropomorphize non-human agents. We propose several key factors that determine anthropomorphism, 
namely the 
design of the non-human agent, the individual characteristics of the human 
user, and the context of use. An initial model that we call 
\textit{``Dynamics of Anthropomorphism''} tries to formalize the qualitative changes in anthropomorphism over time. To 
account for these variances, our model describes the non-linear relationship between anthropomorphic effects of a human-robot interaction and the human user's growing experience with the system.
We also present a cognitive interpretation that considers 
these described dynamics. Further, we discuss scenarios and techniques to measure anthropomorphism. Finally, we make suggestions how others can 
apply our model to better describe the complexity in human's 
...
model of the dynamics, ... // JF: yep, I agree, we should also come back to our initial hypotheses}
...
Of course, the scenario and measurements used depend on the research questions, so a direct human-robot interaction might not always be preferred.
...
Regarding the second issue, how and with which means anthropomorphism is measured, the central point is that ``human-likeness'' itself is complex and manifold. It as a quite diverse and abstract concept, and instead of asking such a vague question, whether a robot is human-like or not, \cite{kahn_jr._robotic_2006} suggest to ask for more concrete constructs that are typical or unique of the concept of ``human-likeness''.
...
explored both rating-scales in a questionnaire, and open questions in an interview (inspired from \citep{kahn_jr._robotic_2006,weiss_i_2009}).
...
One main challenge that robots with applications in daily life contexts face is to sustain long-term interaction. 
...
reached at the end of the novelty effect, and then decreases. A natural question
...
\fixme{JF: Severin, I am not sure what you want to say here.}
