...
To shed light on this phenomenon as a whole, first of all, we would like to propose a clarification of terms.
...
However, the term anthropomorphism should \textit{not} be used to refer to the design of a non-human agent or artifact. This would violate the understanding of anthropomorphism arising in an interaction. We propose to use the term  \textbf{anthropomorphic design} to refer to an imitation of human-like form in the \textit{form} of artificial agents. (In the following, we might use the terms \textit{form} and \textit{design} interchangeably.)

In literature, this distinction is not always done, which is a major source of confusion.\\
...
take two main \textit{perspectives} when trying to explain why humans anthropomorphize \citep{lee_human_2005}: an artifact-centered perspective and a human-centered perspective (see grey triangles in Figure \ref{fig:anthropofig}).
...
\subsubsection{Diverse understandings of anthropomorphism and confusions in the terminology used\\}
...
found\fxwarning{SL: which conflicts are we talking about?}, the HRI
...
\citet{kanda_interactive_2004} point out, however, that relationships among
...
In their study, \citet{kanda_interactive_2004} were also among the first to
...
given situation or interaction, it is implied that one attempts to correct,
adjust, or maybe integrate less accessible information into a more automatically
activated default representation. Epley argues that this process can be seen as
...
Refinements to the initial representation mean something evolves. This can be
due to the fact that a person's knowledge base changes constantly with newly
acquired things, or growing experiences with a robot. Consequently, it is likely
that the ``need to anthropomorphize'' a robot decreases over time (from the
human-centered point of view), since the user's knowledge about the robot
evolves, and also the user familiarizes herself with the robot. Consequently,
the robot should become more predictable and more familiar to the human user and
inferences about it can be made based on the acquired knowledge.\fxwarning{SL:
Rephrase the last part of this paragraph}
...
environment and agents around them. Through this process, uncertainty is
reduced, and consequently, a first unfamiliar robot, can be explained with means
that are different from anthropomorphism.\fxwarning{SL: Paragraph unclear to me.
Rephrase it?}
...
introduced in the following section a novel qualitative model of the evolution
of anthropomorphism over time, \ie a model of the dynamics of anthropomorphism.
...
anthropomorphic effects. Note also that we discuss at
...
combination of human, robot and several other factors we introduce below, and
thus varies (as pictured on Figure~\ref{fig:dynamics}, we suggest besides that
$t_{max}$ actually corresponds to the novelty effect peak). The general
...
\fxwarning{SL: Mention the pre-interaction phase}
...
section~\ref{sec:anthropomorphism}. It has been shown, for instance, that
...
role, like the context in which the interaction takes place. Our model of
...
(real or imagined) contact to a robot. In this stage of pre-interaction, people
form initial expectations toward the robot and imagine how they will use it /
interact with it.

Following the discussion in section~\ref{sec:anthropomorphism}, we build \ICA on
three main factors that determine \emph{a priori} that a robot
will be anthropomorphized:

\begin{enumerate}

    \item ~\emph{Human-centered factor} $S$: The \textbf{personality} and individual
    ...
    \item ~\emph{Robot-centered factor} $R$: The robot's \textbf{design} and how it
    ...
    \item ~\emph{Context-centered factor} $C$: The real or imagined
...
For \textbf{personality}, we suggest to apply \citet{epley_seeing_2007}
psychological \textit{3-factor theory} of anthropomorphism. For instance,
...
the \ICA. Quantify the \emph{level} of anthropomorphic design in a robot, as
...
\fxfatal{SL: Julia, do you have smthg to provide an initial account of what the
novelty effect is?}
...
through his theory of mind\fxwarning{SL: last part not so clear. Rephrase?}.

\fxwarning{SL: mention that the stages are not mutually exclusive and actually
overlap over time.}
