...
Further, we would like to investigate the link between the variances in
anthropomorphism and the correlating cognitive states.
...
is entirely another question, see Section \ref{sec:ethics}).
...
conversely relied on to have the robot itself iteratively adapting its behavior?
In particular, we hypothesize that the so-called novelty effect represents a
peak in anthropomorphic manifestations, followed by mostly deception-driven
refinements of the cognitive behavioral model of the robot.  Could we imagine
that the robot pro-actively \emph{verbalizes} its own limits, in a timely manner
(likely before the end of the novelty effect). And how would this affect the
cognitive models built by the human?
...
After all discussion, there remains the question how far a sustained
anthropomorphic (human social) interaction between a human and a non-human can
be favored, regarding ethical aspects. We need to rethink carefully the
consequences that emotional bonds between humans and robots can have. They might
be beneficial on one hand but can also hold negative implications. How far
should a robot's design encourage us to lastingly project feelings or intentions
to it? Why should a machine even react to us as if it had emotions? Can this be
the reality we want to live in? What happens as soon as the artificial agent is
considered a \textit{moral agent} \citep{sullins_when_2006}? Also, the use of
\textit{socially interactive robots} in contexts involving elderly care or in
rehabilitation and with autistic children is a controversially discussed topic
\cite{robins_robots_2005}. Do we want robots to comfort us; robots that make us
eventually even falling in love with them?  The anthropomorphic design of robots
is critical. Why do we even need this? The HRI / robotics community needs to
