...
Anthropomorphism is a phenomenon that is often studied related to the human-like
design of robots. While people's tendency to perceive human-like 
...
feelings of affinity in the human observer / user. A positive valence reflects
the experience of emotional engagement with and feelings of empathy for the
...
and increase people's acceptance of it.  However, at a certain point, as the
non-human agent approaches but fails to closely resemble a human, the evoked
valence in the human user suddenly turns into a negative uncanny feeling of what
\cite{mori_uncanny_1970} describes as disgust and revulsion. This, in turn,
might lead to rejection of the non-human agent. The Uncanny Valley hypothesis
predicts that with further increasing of the human-likeness such as it appears
so realistic that it is not distinguishable from a human and actually perceived
to be human, the effect is again a positive one, reflecting feelings and
engagement that are similar as those evoked by a human-human interaction.
Developers and designers of artificial agents (\eg virtual characters, robots)
try to exploit the positive effects of \textbf{anthropomorphic form} and avoid
``falling'' into this Uncanny Valley, and consequently, there is a considerable
research effort to better understand the boundaries of anthropomorphic design
and their effects on the interaction and experience. To date, findings have
remained however inconsistent \citep{cheetham_human_2011}.

A phenomenon that is often studied related to the human-like design of objects
and artificial agents is \textbf{anthropomorphism}.  Along this line, this
article aims at fostering a better understanding of anthropomorphism (noted \Ant
in this paper) in human-robot interaction by considering the phenomenon as a
whole. Robotics researchers often tend to believe that anthropomorphism only
describes a static set of human-like features of a robot (like shape, speech
capabilities or facial expressions). We refer to these characteristics as the
\emph{anthropomorphic design} of the robot~\citep{fink_anthropomorphism_2012},
and propose that \emph{anthropomorphism} in general refers to a \emph{social
phenomenon} that emerges from the (real or imagined) \emph{interaction} between
a non-human agent (\eg a robot) and a
human~\citep{persson_anthropomorphism_2000}. According
...
To support the reflection on variances and dimensions of anthropomorphism, it
may be useful to first consider a few experimental results. We studied
anthropomorphism in several short- and long-term human-robot interaction studies
that we carried out with embodied robots, both in ecologically valid real-world
environments and controlled lab settings. Those experiments are presented in
details in previously published articles~\citep{fink_anthropomorphic_2012,
fink_living_2013, fink2014which, lemaignan2014dynamics}. We only highlight
here a few qualitative results that underline the complexity of the
phenomenon.
...
their home. At various points in time, users were asked to rate their perception
of different features of the robot on a 7-point rating scale. As
...
understand the underlying psychological phenomenons that explain the decrease of
interest, and can we effectively account for the dynamics of these phenomenons?

Those questions are certainly not new, but paradoxically, while anthropomorphism
is a commonly discussed trait of human-robot interaction, it appears that we
still lack formal grounds and models that would account for the long-term
...
\begin{figure}[h!]
    ...
    number of interactions and the qualitative anthropomorphic projections. $n = 13$ groups}
        ...
        behavior and their perception of the robot were found but were
...
way that it supports improved design of human-robot interactions?

To this end, we propose to discuss anthropomorphism from three complementary
perspectives: 
...
\item ~A person's tendency to anthropomorphize a robot
    ...
    anthropomorphizes the robot less: \textit{anthropomorphism is a dynamic
    phenomenon}.
...
considerations and the proposed model.  This is followed (Section
\ref{sec:discussion}) by a discussion about the limitations of our approach and
the existing and alternative ways to measure anthropomorphism in human-robot
interactions.  Finally, Section \ref{sec:conclusion} concludes this article and
offers starting points for further research on anthropomorphism and the effect
of the human-like design approach in robotics.
...
\begin{figure}[b]
    ...
    \caption{what is anthropomorphism? \hl{we need a cool caption for this that includes terms such as factor, element, influence, etc. :-) }}
...
theories from developmental psychology, cognitive science, and social psychology to explain people's tendency to 
...
studied trait of human-robot interaction; however its current understanding seems to be stuck in a twofold consideration. Mostly, only two key factors are recognized: the
...
Through applying an understanding that anthropomorphism represents a specific type of \textit{experience} that arises in the
...
we propose that
anthropomorphism is a multi-layered phenomenon which arises in different levels \citep{persson_anthropomorphism_2000}, and which is likely to evolve with continued interaction.
...
To this end, and in respect of our research hypotheses, we make the following two suggestions:
...
Second, we would like to take up the proposition already made by other researchers (\eg \cite{persson_anthropomorphism_2000,takayama_perspectives_2012,ruijten_introducing_2014}) to not viewing anthropomorphism as being either there or not there, but to discriminate different levels of
...
With having both before-mentioned aspects in mind (multi-factors that determine anthropomorphism and multi-levels of the observable phenomenon), it becomes clear that anthropomorphism cannot be something fixed, that once observed in a human-robot interaction always remains the same. We think, that anthropomorphism evolves and shows dynamics. This idea is what we would like to formalize in the next section.
...
\emph{adopting} a robot, for instance). Though conflicts were found, the HRI community more or less accepted viewing anthropomorphism as a static feature that once
observed during a short-term interaction reflects a sustaining social effect. So far, it was also measured as such. But there are some hints that this is deficient.

For instance, \cite{kanda_interactive_2004} point out that relationships among people evolve over time and similarly, also people’s attitude toward technological artifacts
and their relationship with them might evolve over time. In their study, \cite{kanda_interactive_2004} were also among the first to describe empirically \emph{novelty effects} during a long-term study with an interactive robot in a school.

There are also hints from a more theoretical side, that make us think that anthropomorphism is likely to change over 
time and with growing experience that a person has with a robot.

%effectance motivation reduces uncertainty and consequently the tendency to anthropomorphize
For instance, let's have a look at the processes that underlie anthropomorphism.
In its essence anthropomorphism means reasoning about and perceiving something
non-human (and unknown or unfamiliar) based on one's representation of the
familiar and well-known concept of being human (or one-self). According to \citep{epley_when_2008} the basic
operations that underlie these inductive inferences are the acquisition of knowledge, the
activation or elicitation of knowledge, and the application of activated
knowledge at the time of the judgment. By applying knowledge to a given situation / interaction, it is implied that one attempts to correct, adjust, or maybe
integrate less accessible information into a more automatically activated
default representation. \cite{epley_when_2008} argues that this process can be seen as a process of correction, and
thus, it is often insufficient leaving final judgments biased in the direction
of the initially activated representation. Refinements to the initial representation mean something evolves. This can be due to the fact that a person's knowledge base changes
constantly with newly acquired things, or growing experiences with a
robot, \etc. . Consequently, it is likely that the ``need to anthropomorphize'' a robot decreases
over time (from the human-centered point of view), since the user's knowledge about the robot evolves, and also the user familiarizes herself with the robot. Consequently, the robot should become more
predictable and more familiar to the human user and inferences about it can be
made based on the acquired knowledge.

In other words, the argumentation is that humans have a motivation to interact effectively in their environment \textit{(effectance)} \citep{epley_when_2008}. This means that humans are motivated to be able to understand and predict the environment and agents around them. Through this process, uncertainty is reduced, and consequently, a first unfamiliar robot, can be explained with means that are different from anthropomorphism.
...
HRI2013 paper}
This is supported by the results from \citet{Waytz2010} seen in section
