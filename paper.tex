...
\end{tikzpicture}
%}
\caption{\textbf{The three cognitive layers of anthropomorphism}: Layer 1 is the
instinctive, sub-cognitive identification of living peers. {\it Empathy} is
characteristic of this stage. The Layer 2 materializes after longer
observation or short, non-contextualized interaction (typically, a lab
environment): the user projects a mental model he/she is already familiar
with onto the robot. After longer {\it contextualized} interaction
(typically, at home), the user enters Layer 3 of anthropomorphism: the user
composes a custom mental model of the robot, based on experience. This leads
to adapted interaction modalities, that may still be anthropomorphic, or
not. Note that the different layers coexist: Layer 1 and 2 continue to affect the
human perception of the robot in a Layer 3 interaction.}
...
\draw(0.5,6.0) node[anchor=south west] {\scriptsize \sc Init.};
\draw(2.5,6.0) node[anchor=south west] {\scriptsize \sc Familiarization};
\draw(7,6.0) node[anchor=south west] {\scriptsize \sc Stabilization};
...
\draw (-0.05, 2.1) -- (0.05, 2.1) node[anchor=east] {\IPAe};
\draw (1.8, 4.9) -- (1.8, 5.1) node[anchor=south] {\scriptsize \AntEMax};
\draw (10, 0.8) node[anchor=west] {\SLAe};
...
\caption{\textbf{Towards a cognitive interpretation of the dynamics of
    ...
    and~\ref{fig:cognitivemodel} to evidence their interplay: 
    the cognitive layers 1 and 2 (pre-cognitive anthropomorphism and projection
    of known existing cognitive models) play a key role in the early
