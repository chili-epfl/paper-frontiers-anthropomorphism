...
In this section we will clarify the terminology that we use as we have found conflicting understandings of the term ``anthropomorphism'' in the related literature. We first present anthropomorphism as a social phenomenon. Then we propose to use another term, namely ``anthropomorphic form'', to refer to the human-inspired design of artifacts. Finally, we focus on anthropomorphism in Human-Robot Interaction, as it naturally brings together (and needs to question) the usage of an anthropomorphic design of robots and its consequences for the interaction with the human user. 


\subsection{Anthropomorphism as a Social Phenomenon}
\label{sec:social-phenomenon}

Anthropomorphism is an interesting phenomenon because it tells us something about the relationship between the human who anthropomorphizes, and the non-human agent which is anthropomorphized. Though commonly observed, it appears controversial, at the same time, that humans tend to perceive human-like physical appearance (\eg faces), emotional states, or inner mental states and motivations in non-human agents such as animals, natural forces, religious agents, technological gadgets, or mechanical devices \citep{epley_when_2008}.\footnote{According to \cite{epley_when_2008}, a non-human agent can be anything that acts -- or is believed to act -- with apparent independence.}  Anthropomorphism has received remarkable attention across several domains, such as natural sciences, psychology, Human-Computer Interaction, and Human-Robot Interaction. However, the term is used in different senses throughout these disciplines \citep{duffy_anthropomorphism_2003} which makes it difficult to synthesize findings.

Originally, \textbf{anthropomorphism} comes from the Greek \textit{`anthropos'} for `man' (or `human') and \textit{`morphe'} for `form / structure' (or `shape'). It can generally be understood as people's tendency to perceive a non-human agent as if it had some human characteristics. In turn, how a person perceives an agent further determines how she relates to and behaves towards the agent, or how she may behave in light of this agent \citep{epley_when_2008}. By this, anthropomorphism can be seen as a special kind of relationship between the human and the non-human agent. This relationship is often characterized by the fact that the human attributes or ascribes uniquely human characteristics to the non-human entity or artifact. The most prominently anthropomorphized non-human agents are probably animals and robots \citep{duffy_anthropomorphism_2003,schmitz_concepts_2011}.

Apart from anthropomorphism there exists also \textbf{animism}, which can be understood as the \textit{"attribution of life to the nonliving"} \hl{in Schmitz 2011, cite Piaget original text!}. Using this definition, animism differs in two main aspects from anthropomorphism:
First, the property which is ascribed differs: it is not human-unique but applies to all living beings (life-likeness). For instance, attributing life-likeness to a stone must not necessarily be labeled as anthropomorphism, since being alive is not a human unique property but applies to all living beings.
The second difference comes from the instance (or better things) to which life is attributed: all living entities are excluded in this definition. In other words, pets as non-human but living beings can only be anthropomorphized (because they are not human) but not animized (because they are already alive).
Often synonymously used to animism is the term \textbf{zoomorphism}, which applies to the cases when non-lifelike instances / things are associated with animalistic attributes, excluding human-specific traits. Both anthropomorphism and zoomorphism can be embraced by the concept of \textbf{life-likeness}. \footnote{The inverse process would be called dehumanization \cite{haslam_dehumanization:_2006}, or mechanomorphism \cite{caporael_anthropomorphism_1986}.}
